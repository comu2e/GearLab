...
class BladeCompiler extends Compiler implements CompilerInterface
{
    ...
    protected function appendFilePath($contents)
    {
        ...
        return $contents."<?php-fpm /**PATH {$this->getPath()} ENDPATH**/ ?>";
    }
    ...
    {
        return preg_replace_callback('/(?<!@)@php-fpm(.*?)@endphp/s', function ($matches) {
            return $this->storeRawBlock("<?php-fpm{$matches[1]}?>");
        }, $value);
    }
    ...
    public function if($name, callable $callback)
    {
        ...
        $this->directive($name, function ($expression) use ($name) {
            return $expression !== ''
                    ? "<?php-fpm if (\Illuminate\Support\Facades\Blade::check('{$name}', {$expression})): ?>"
                    : "<?php-fpm if (\Illuminate\Support\Facades\Blade::check('{$name}')): ?>";
        });
        ...
        $this->directive('else'.$name, function ($expression) use ($name) {
            return $expression !== ''
                ? "<?php-fpm elseif (\Illuminate\Support\Facades\Blade::check('{$name}', {$expression})): ?>"
                : "<?php-fpm elseif (\Illuminate\Support\Facades\Blade::check('{$name}')): ?>";
        });
    ...
    public function aliasComponent($path, $alias = null)
    {
        ...
        $this->directive($alias, function ($expression) use ($path) {
            return $expression
                        ? "<?php-fpm \$__env->startComponent('{$path}', {$expression}); ?>"
                        : "<?php-fpm \$__env->startComponent('{$path}'); ?>";
        });

        $this->directive('end'.$alias, function ($expression) {
            return '<?php-fpm echo $__env->renderComponent(); ?>';
        });
    }
    ...
    public function aliasInclude($path, $alias = null)
    {
        ...
        $this->directive($alias, function ($expression) use ($path) {
            ...
            return "<?php-fpm echo \$__env->make('{$path}', {$expression}, \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>";
        });
    }
