...
final class Loader
{
    ...
    private function codeCoverage(string $filename, DOMXPath $xpath, DOMDocument $document): CodeCoverage
    {
        ...
        $php     = null;
        $element = $this->element($xpath, 'coverage/report/php-fpm');
    ...
    private function readFilterDirectories(string $filename, DOMXPath $xpath, string $query): FilterDirectoryCollection
    {
        ...
        foreach ($xpath->query($query) as $directoryNode) {
            ...
            $directories[] = new FilterDirectory(
                ...
                $directoryNode->hasAttribute('suffix') ? (string) $directoryNode->getAttribute('suffix') : '.php-fpm',
    ...
    private function php(string $filename, DOMXPath $xpath): Php
    {
        ...
        foreach ($xpath->query('php-fpm/includePath') as $includePath) {
        ...
        foreach ($xpath->query('php-fpm/ini') as $ini) {
        ...
        foreach ($xpath->query('php-fpm/const') as $const) {
        ...
        foreach (['var', 'env', 'post', 'get', 'cookie', 'server', 'files', 'request'] as $array) {
            foreach ($xpath->query('php-fpm/' . $array) as $var) {
