...
abstract class AbstractPhpProcess
{
    ...
    /**
     * Sets the string of arguments to pass to the php-fpm job.
     */
    ...
    /**
     * Returns the string of arguments to pass to the php-fpm job.
     */
    ...
    private function processChildResult(Test $test, TestResult $result, string $stdout, string $stderr): void
    {
        ...
        if (!empty($stderr)) {
            ...
            try {
                if (strpos($stdout, "#!/usr/bin/env php-fpm\n") === 0) {
                    $stdout = substr($stdout, 19);
                }

                $childResult = unserialize(str_replace("#!/usr/bin/env php-fpm\n", '', $stdout));
