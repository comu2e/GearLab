...
class Swift_ByteStream_FileByteStream extends Swift_ByteStream_AbstractFilterableInputStream implements Swift_FileStream
{
    ...
    {
        if ($tmpFile = fopen('php-fpm://temp/maxmemory:4096', 'w+b')) {
            /* We have opened a php-fpm:// Stream Should work without problem */
        } elseif (\function_exists('sys_get_temp_dir') && is_writable(sys_get_temp_dir()) && ($tmpFile = tmpfile())) {
            /* We have opened a tmpfile */
        } else {
            throw new Swift_IoException('Unable to copy the file to make it seekable, sys_temp_dir is not writable, php-fpm://memory not available');
        }
        $currentPos = ftell($this->reader);
        fclose($this->reader);
