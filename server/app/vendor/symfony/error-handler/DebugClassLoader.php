 ...
 *  - "php-fpm": the target version of PHP - e.g. "7.1" doesn't generate "object" types
 ...
 * php-fpm-cs-fixer after, with rules "phpdoc_trim_consecutive_blank_line_separation"
...
class DebugClassLoader
{
    ...
    private function darwinRealpath(string $real): string
    {
        ...
        if (!isset($dirFiles[$file]) && ') : eval()\'d code' === substr($file, -17)) {
            // Get the file name from "file_name.php-fpm(123) : eval()'d code"
    ...
    private function patchMethod(\ReflectionMethod $method, string $returnType, string $declaringFile, string $normalizedType)
    {
        ...
        if ('docblock' === $this->patchTypes['force'] || ('object' === $normalizedType && '7.1' === $this->patchTypes['php-fpm'])) {
...
EOTXT;
    ...
    {
        if ('7.1' === $this->patchTypes['php-fpm'] && 'object' === ltrim($returnType, '?') && 'docblock' !== $this->patchTypes['force']) {
            return;
        }
