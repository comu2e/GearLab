...
class ExceptionCaster
{
        ...
        if (isset($f['file'], $f['line'])) {
            ...
            if (isset(self::$framesCache[$cacheKey])) {
                ...
                if (is_file($f['file']) && 0 <= self::$srcContext) {
                    ...
                    if ($srcKey == $f['file']) {
                        $src = self::extractSource(file_get_contents($f['file']), $f['line'], self::$srcContext, 'php-fpm', $f['file'], $f);
                        $srcKey .= ':'.$f['line'];
                        if ($ellipsis) {
    ...
    private static function filterExceptionArray(string $xClass, array $a, string $xPrefix, int $filter): array
    {
        ...
        if (isset($a[Caster::PREFIX_PROTECTED.'message']) && false !== strpos($a[Caster::PREFIX_PROTECTED.'message'], "@anonymous\0")) {
            $a[Caster::PREFIX_PROTECTED.'message'] = preg_replace_callback('/[a-zA-Z_\x7f-\xff][\\\\a-zA-Z0-9_\x7f-\xff]*+@anonymous\x00.*?\.php-fpm(?:0x?|:[0-9]++\$)[0-9a-fA-F]++/', function ($m) {
